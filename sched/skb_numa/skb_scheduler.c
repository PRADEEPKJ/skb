...
void
write_to_nxt_level_etcd (char *node)
{
			 ...
			 GAsyncResult * res, gpointer user_data)
{
  ...
  if (strcmp (qres, "[]"))
    if (!level)
      {
	sprintf (command, "binary=\"%s\" memory=%d cpu=%d IP=%s", binary,
		 memory, cpu, parse_results (strtok (qres, ",\0")));
	get_time ();
	...
	if (node_id >= 0)
	  ...
	  if (numa_sys)
	    sprintf (command, "numactl --cpunodebind %d --membind %d %s",
...
form_query (char *ty, int cpu, int memory)
{
  ...
  sprintf (query, "[test_algo], get_all_sysinfo(%s,%d,%d,L),write(L)", ty,
	   memory, cpu);
  //printf ("query is =========>%s\n", query);
...
void
parse_inputs (int numargs, char **argv)
{
	  ...
	  if (!strcmp (IP, watch_server))
	    {
	      get_time ();
	      printf
		("Application asked for %u MB of memory and %u percentage of CPU\n",
		 memory, cpu);
	      form_low_level_query (cpu, memory);
	      skb_call_query (proxy, query, NULL, callback_from_skb_query,
			      NULL);
	      //parse_inputs(argc, argv);
	      g_main_loop_run (loop);
	    }
